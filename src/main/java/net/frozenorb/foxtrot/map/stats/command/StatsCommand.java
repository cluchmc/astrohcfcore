...
public class StatsCommand {
    ...
    public static void stats(CommandSender sender, @Param(name = "player", defaultValue = "self") UUID uuid) {
        ...
        sender.sendMessage(ChatColor.DARK_RED + UUIDUtils.name(uuid));
        sender.sendMessage(ChatColor.GRAY.toString() + ChatColor.STRIKETHROUGH + StringUtils.repeat('-', 53));

        sender.sendMessage(ChatColor.RED + "Kills: " + ChatColor.WHITE + stats.getKills());
        sender.sendMessage(ChatColor.RED + "Deaths: " + ChatColor.WHITE + stats.getDeaths());
        if (Foxtrot.getInstance().getMapHandler().isKitMap() && !Foxtrot.getInstance().getServerHandler().isVeltKitMap()) {
            sender.sendMessage(ChatColor.RED + "Killstreak: " + ChatColor.WHITE + stats.getKillstreak());
            sender.sendMessage(ChatColor.RED + "Highest Killstreak: " + ChatColor.WHITE + stats.getHighestKillstreak());
        }
        sender.sendMessage(ChatColor.RED + "KD: " + ChatColor.RED + (stats.getDeaths() == 0 ? "Infinity" : Team.DTR_FORMAT.format((double) stats.getKills() / (double) stats.getDeaths())));
